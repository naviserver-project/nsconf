...
# $Header: /cvsroot/naviserver/modules/nsconf/nsconf.tcl,v 1.5 2008/03/27 16:49:12 seryakov Exp $
...
if { ![nsv_exists _ns_config server.$server] } {
  ...
  nsv_set _ns_config help.ns/parameters.jobtimeout "Number of seconds for a job thread to wait for new task, in case of timeout thread will exit"
  nsv_set _ns_config help.ns/parameters.jobsperthread "Max number of Tcl jobs to execute inside the job queue thread before thread exit"
  nsv_set _ns_config help.ns/parameters.schedsperthread "Max number of tasks to execute inside the schedule thread before thread exit"
...
proc _ns_config.info {} {
    ...
    if {[info command ns_perm] != "" } {
      append html "</td></tr>
          <tr valign=top><td class=lbl>ns_perm: Users:</td><td>"

      foreach { user pwd d } [ns_perm listusers] {
        append html $user ":" $pwd ":::::<br>"
      }        

      append html "</td></tr>
          <tr valign=top><td class=lbl>ns_perm: Groups:</td><td>"

      foreach { group users } [ns_perm listgroups] {
        append html $group ":" [join $users ,] "<br>"
      }        

      append html "</td></tr>
          <tr valign=top><td class=lbl>ns_perm: Permissions:</td><td>"

      foreach perm [ns_perm listperms] {
        append html $perm "<br>"
      }        
      
      append html "</td></tr>
          <tr valign=top><td class=lbl>ns_perm: <A HREF=?@page=reloadnsperm>Reload</A></td><td>"
    }
...
proc _ns_config.reloadnsperm {} {

    if { [info command ns_permreload] != "" } {
      ns_permreload
    }

    set html [_ns_config.header "ns_perm files have been reloaded"]
    append html "<script>setTimeout('window.location=\"?@page=info\"',1000)</script>"
    append html [_ns_config.footer]
